...
namespace overlay_ui {
  struct Vec {
    ...
    explicit constexpr Vec(float x, float y): x(x), y(y) { }
    explicit constexpr Vec(): x(0), y(0) { }
  ...
  struct Rect {
    ...
    explicit constexpr Rect(): left(0), top(0), right(0), bottom(0) { }
  ...
  struct NodeImpl : Node {
    virtual void visit(NodeDrawVisitor& visit) { visit(*static_cast<Impl*>(this)); }
    virtual void visit(NodeLayoutVisitor& visit) { visit(*static_cast<Impl*>(this)); }
  };
  ...
  inline void update(Layer& layer, NodeImpl &node, bool sticky, bool lazySnap = false) {
    ...
    if (!overlay_ui::drawable(frame)) return;
